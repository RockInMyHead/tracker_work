...
const NAVIGATION: NavigationItem[] = [
  ...
  { name: 'Нагрузка', href: '/workload', icon: ChartBarIcon },
  { name: 'Важные', href: '/important', icon: ExclamationTriangleIcon },
];
...
const UserInfo: React.FC<{ user?: User | null }> = ({ user }) => (
  <div className="hidden lg:flex items-center space-x-3 bg-white/10 rounded-xl px-4 py-2">
    ...
    <div>
      ...
      <div className="text-xs text-indigo-200">Администратор</div>
    </div>
  </div>
);
...
export const Header: React.FC<HeaderProps> = ({ isAuthenticated, user, onLogout }) => {
  ...
  return (
    <header className="bg-indigo-600 shadow-lg border-b border-indigo-300 sticky top-0 z-50">
